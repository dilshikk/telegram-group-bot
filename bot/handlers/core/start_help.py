...
_START_TEXT = (
    "👋 Привет!\n\n"
    "Group Help — наиболее полный бот для управления вашими группами!\n\n"
    "👉 Добавьте меня в супергруппу и сделайте Администратором.\n\n"
    "❓ Команды — нажмите /help"
)

_SETTINGS_INFO_TEXT = (
    "⚙️ <b>Настройки группы</b>\n\n"
    "Используйте команду /settings в своей группе.\n\n"
...
_INFO_TEXT = (
    ...
    "Group Help — многофункциональный бот для управления группами.\n\n"
    "<b>Возможности:</b>\n"
    "• Антиспам и антифлуд\n"
...
_HELP_CATEGORIES_TEXT: dict[str, str] = {
    ...
    "clone": (
        ...
        "Оркестратор запустит бота в течение минуты."
    ),
}
...
async def _forward_settings_to_pm(message: Message) -> bool:
    """
    Отправляет меню настроек в ЛС администратора.
    chat_id группы встроен прямо в клавиатуру — никакого FSM и Redis не нужно.
    ...
    try:
        await message.bot.send_message(
            ...
            reply_markup=_main_keyboard(0, chat_id),  # chat_id в кнопках!
            parse_mode="HTML",
        )
        return True
...
async def cmd_start(message: Message) -> None:
    ...
    if sent:
        ...
        await message.answer(
            "⚠️ Сначала запустите бота в личном чате!",
...
async def cmd_reload(message: Message) -> None:
    ...
    if sent:
        await message.answer(
            "✅ Меню настроек обновлено и отправлено в личный чат.",
            reply_markup=_go_to_pm_keyboard(username),
        )
    else:
        await message.answer(
            "⚠️ Сначала запустите бота в личном чате, затем повторите /reload.",
            reply_markup=_start_bot_keyboard(username),
        )
...
async def cmd_help(message: Message) -> None:
    await message.answer("❓ Выберите категорию:", reply_markup=_help_keyboard())
...
async def cb_help_back(call: CallbackQuery) -> None:
    await call.message.edit_text(
        "❓ Выберите категорию:", reply_markup=_help_keyboard()
    )
    await call.answer()
...
async def cb_help_category(call: CallbackQuery) -> None:
    ...
    text = _HELP_CATEGORIES_TEXT.get(key, _HELP_CATEGORIES_TEXT["basic"])
